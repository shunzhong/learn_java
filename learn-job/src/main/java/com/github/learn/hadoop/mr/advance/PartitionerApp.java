package com.github.learn.hadoop.mr.advance;
...
public class PartitionerApp extends Configured implements Tool{
	...
	public static void main(String[] args) throws Exception{
		...
		ToolRunner.run(conf, new PartitionerApp(), args);
	}
	
	public int run(String[] args) throws Exception {
		
		Job job = Job.getInstance(getConf(), PartitionerApp.class.getSimpleName());
 ...
 class TrafficPartitioner extends Partitioner<Text, TrafficWritable> {
	
	@Override
	public int getPartition(Text key2, TrafficWritable value2, int numPartitions) {
		return key2.toString().length()==11?0:1;
	}
}
