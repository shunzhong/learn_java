...
public class SelectSockets {
	 ...
	 * by * the selector to be ready for reading. If the channel returns 142 an
	 * EOF condition, it is closed here, which automatically * invalidates the
	...
	protected void readDataFromSocket(SelectionKey key) throws Exception {
		...
		while ((count = socketChannel.read(buffer)) > 0) {
			...
			// the same nonblocking
			// channel it read the data from, this code can // potentially spin
			// in a busy loop. In real life
			// you'd do something more useful than this. buffer.clear();
			// Empty buffer
		}
		if (count < 0) {
